...
% -------------------- Debug 模式：只跑 Detect + Track + Debug 输出 --------------------
if isfield(Cfg,'Debug') && isfield(Cfg.Debug,'Enable') && Cfg.Debug.Enable
    % Debug 语义：用于快速检查识别/追踪效果，不继续运行计算/过滤/分析等后续步骤
    return;
end
...
if isfield(Cfg,'Compute') && isfield(Cfg.Compute,'Enable') && Cfg.Compute.Enable
    ...
    try
        ...
        guvCompute_exportAllResultsCSV(AllResults, outCsv, Paths.SeriesName, SeriesID, Info.CNames);
    catch ME
        warning('[%s] Export AllResults.csv failed: %s', Paths.SeriesName, ME.message);
    end

    % 说明：结果人工检查（VisualizeSingleID）已从主流程摘出，
    % 请使用独立脚本包 VisualizeSingleID_Pipeline.zip 在计算后人工挑选 ID 进行绘图。
end
