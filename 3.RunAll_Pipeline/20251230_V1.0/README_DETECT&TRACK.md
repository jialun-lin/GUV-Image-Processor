...
```matlab
Cfg = guvPipeline_configDefault();   % functions/Pipeline/
MasterTable = GUV_Pipeline(Cfg);     % 根目录入口（自动 addpath(genpath)）
```

### 4.2 你最常改的配置项（`functions/Pipeline/guvPipeline_configDefault.m`）
...
本包已把你提供的 `GUV-Image-Processor-main` 两个脚本封装为两个**单文件大函数**并接入 Pipeline：

* `functions/Compute/guvCompute_calcAllResultsFromTracks.m`
* `functions/Compute/guvCompute_visualizeAllResults.m`
...
输出位置：每个 `XY###` 目录下新增 `Computation/`（含 `AllResults.mat`）与 `Computation/Figures/`（png）。
...
OutRoot/
  XY003/
    ...
    TTracks.mat
    XY003_Table.csv
OutRoot/GUV_MASTER_DB.csv
OutRoot/GUV_MASTER_DB.mat
```
...
```matlab
T = readtable(fullfile(Cfg.OutRoot,'GUV_MASTER_DB.csv'));
