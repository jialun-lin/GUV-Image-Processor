...
if isfield(GUVDataRef,'bboxes') && isfield(GUVDataRef,'subMasks')
    innerMean = guvMatch_meanOnBBoxMasks(I_norm, GUVDataRef.bboxes, GUVDataRef.subMasks);
end

% mem：轮廓环带 memBandMasks
memMean = nan(n,1);
if isfield(GUVDataRef,'bboxes') && isfield(GUVDataRef,'memBandMasks')
    memMean = guvMatch_meanOnBBoxMasks(I_norm, GUVDataRef.bboxes, GUVDataRef.memBandMasks);
end
